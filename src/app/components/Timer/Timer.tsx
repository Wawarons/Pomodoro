...
const Timer = () => {
    ...
    useEffect(() => {
        ...
        if(timer.sessionStart)  {
            ...
            if(timeLeft && !timer.isBreak){
                ...
                if(timer.stepSession === 'work')
                    updateTimer({stepSession: ((timer.sessionDo+1)%4 === 0 ? "long break":"short break"), sessionDo: timer.sessionDo + 1, isBreak: true});
                else
                    updateTimer({stepSession: 'work', isBreak: true});
            }
        }else{
    ...
    function millisToMinutesAndSeconds(secondsLeft: number): {minutes: number|string, seconds: number|string} {
    ...
    return (
        ...
        <div className={style['time-container']}>
            <h2 className={style['time-container__title--session']}>{timer.stepSession}</h2>
            <div className={style['timer']}>
            <div>
                <h2 className={style['time-value']}>{millisToMinutesAndSeconds(timeLeft).minutes}</h2>
                <h2 className={style['time-value']}>{millisToMinutesAndSeconds(timeLeft).seconds}</h2>
            </div>
            </div>
        </div>
